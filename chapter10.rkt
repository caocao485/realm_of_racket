...
;; Game States and Game Trees for Dice of Doom 
...
(define (even-row pos top? bottom? right? left?)
  ...
  (append (add (or top? right?) (add1 (- pos BOARD)))
          ...
          (add right?(add1 pos))
          (add left?  (sub1 pos)))) ;;error

(define (odd-row pos top? bottom? right? left?) ;;todo
  (append (add (or top? right?) (add1 (- pos BOARD)))
          (add (or bottom? right?) (add1 (+ pos BOARD)))
          (add top? (- pos BOARD))
          (add bottom? (+ pos BOARD))
